...
func (q *CreateUserQuery) Scan(row pgx.Row) error {
	return row.Scan(&q.Result.ID, &q.Result.Name, &q.Result.Email, &q.Result.Status, &q.Result.Description, &q.Result.CreatedAt)
}
...
func (q *ListUsersQuery) ScanRow(row pgx.Row) error {
	var i User
	if err := row.Scan(&i.ID, &i.Name, &i.Email, &i.Status, &i.Description, &i.CreatedAt); err != nil {
		return err
	}
